#include "ParticleEmittor.h"
#include "gl_core_4_4.h"
#include "GLFW\glfw3.h"
#include "Particle.h"
#include "Assets\Camera\FlyCamera.h"

ParticleEmittor::ParticleEmittor()
: m_particles(nullptr),
m_firstDead(0),
m_maxParticles(0),
m_position(0, 0, 0),
...
void ParticleEmittor::initalise(unsigned int a_maxParticles,
...
void ParticleEmittor::update(float a_deltaTime, const glm::mat4& a_cameraTransform)
{
	...
	while (m_emitTimer > m_emitRate) {
		...
		for (unsigned int i = 0; i < m_firstDead; i++)
		{
			...
			particle->lifestime += a_deltaTime;
			if (particle->lifestime >= particle->lifespan)
			{
			...
			else
			{
				...
				particle->size = glm::mix(m_startSize, m_endSize,
					particle->lifestime / particle->lifespan);
				// colour particle
...
void ParticleEmittor::draw()
{
	//sync the particle vretex buffer
	// based of how many alvie particles there are 
	assert(m_vbo > 0);
	glBindBuffer(GL_ARRAY_BUFFER, m_vbo);
