...
ObjectLoader::ObjectLoader(string objName, std::vector<Vertex>& outVertices) {
	indexChecked = false;
	isQuads = false;
	...
	for (GLuint i = 0; i < m_object->s_vertices.size(); ++i)
	{
		...
		vert.position = glm::vec4(m_object->s_vertices[i], 1.0);

		if (i > m_object->s_normals.size() - 1)
			hasUV = false;

		if (!hasUV && !hasNormals)
		{
			vert.normal = glm::vec3(0.0f, 1.0f, 0.0f);
			vert.colour = glm::vec4(0.5f, 0.5f, 0.5f, 1.0f);
		}
